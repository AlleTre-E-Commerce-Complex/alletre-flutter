...
class UserService {
  final String baseUrl = 'http://192.168.132.182:3001/api/auth';
        ...
        try {
          response = await http.post(
            ...
            headers: {
              'Content-Type': 'application/json',
              'Accept': 'application/json',
              'Origin': 'http://192.168.132.182:3001',
  ...
  Future<Map<String, dynamic>> makeDefaultAddress(String locationId) async {
    try {
      ...
      final response = await http.post(
        Uri.parse(
            'http://192.168.132.182:3001/api/users/locations/$locationId/make-default'),
        headers: {
          'Authorization': 'Bearer $accessToken',
          'Content-Type': 'application/json',
        },
